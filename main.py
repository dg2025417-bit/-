...
# ─────────────────────────────────────────────
# 그래프를 그려주는 함수 (가로 길이 줄이기)
# ─────────────────────────────────────────────
def 그래프그리기(지표이름):
    # 화면을 3칸으로 나눠 가운데(4)에만 그래프를 넣어 가로 길이를 줄임
    여백왼, 가운데, 여백오 = st.columns([2, 2, 2])
    with 가운데:
        if 그래프종류 == "막대그래프":
...
# ─────────────────────────────────────────────
st.info(
    "💡 30분 단위로 보면 변화가 더 자세히 보여요!\n\n"
    "- 사용 시간이 늘수록 '수면시간'이 꾸준히 줄어드나요?\n"
    "- 특정 시간대부터 '스트레스'가 급격히 변하나요?\n"
